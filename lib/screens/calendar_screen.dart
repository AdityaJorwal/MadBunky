    ...
    if (useGemini) {
      ...
      if (shouldExtract == true) {
        ...
        try {
          ...
          final recentHistory = history.where((s) {
            return s.startTime.isAfter(sevenDaysAgo) && s.startTime.isBefore(now);
          }).toList();

          final result = await GeminiService.instance.extractSchedule(
            file: File(filePath),
            mimeType: mimeType,
            apiKey: apiKey,
          ...
          final filteredSessions = await showBatchSelectorAndFilter(context, result);
          if (!mounted) return;
          if (filteredSessions.isEmpty) {
            MainScaffold.showGlassToast(context, "No classes extracted or remaining after batch filtering.", isError: true);
            return;
          }
          ...
          showMorphDialog(
            context: context,
            builder: (c) => PdfConfirmationDialog(
              extractedSessions: filteredSessions,
